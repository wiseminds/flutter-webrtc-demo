...
import '../models/signalling_state.dart';
import '../models/video_source.dart';
import 'app_websocket.dart';

typedef CreateStream = Future<MediaStream> Function(
    String media, bool userScreen);

class Signaling {
  final String selfId, roomId, host, peerId;
  ...
  Future<void> connect() async {
    var url = 'wss://localhost:4443?roomId=$roomId&peerId=$peerId';
    // 'wss://$_host:$_port/?roomId=$roomId&peerId=$peerId';

    _socket = AppWebSocket(url);
    ...
    _socket?.onOpen = () {
      ...
      _send('connectionrequest', {'roomId': roomId, 'peerId': peerId});
    };
    ...
    print('connect to socket, ${_socket.toString()}');
  ...
  Future<dynamic> request(event, data) async {
    return _socket?.request(event, data); //_encoder.convert(request));
  }
